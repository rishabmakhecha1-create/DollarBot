...
function setStatus(message, isLive) {
  ...
  apiStatus.style.color = isLive ? "var(--primary-deep)" : "var(--muted)";
}
...
function closeChatDock() {
  ...
  chatToggleButton.textContent = "Open Chat";
...
DollarBot take:
- Keep needs for essentials like transportation, phone, groceries, and school basics.
- Use wants for fun spending so it stays guilt-free and controlled.
- Move savings first, right after payday, so it actually happens.
...
DollarBot take:
Gross pay is what you earned before taxes. Take-home pay is what should actually hit your account.
...
function getReply(input) {
  ...
  if (hasAny(text, ["budget", "budgeting"])) {
    ...
    return buildCoachingReply(
      ...
      detailedBudget
        ? "Use the Budget Planner above and then tell me which category feels too tight."
  ...
  if (hasAny(text, ["paycheck", "w-2", "w2", "tax", "taxes", "withholding"])) {
    return buildCoachingReply(
      ...
      "Use the Paycheck Estimator above or send me your hourly rate and hours so we can break it down."
    );
  }
