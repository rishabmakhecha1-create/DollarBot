...
const STORAGE_KEYS = {
  accounts: "dollarbot.accounts",
  session: "dollarbot.session"
};
...
function setTurnstileStatus(message, isReady = false) {
  turnstileStatus.textContent = message;
  turnstileStatus.style.color = isReady ? "var(--secondary)" : "var(--muted)";
}
...
function setAuthMessage(message, isError = false) {
  ...
  authMessage.style.color = isError ? "#ffb3cb" : "var(--muted)";
}

function showAuthMode(mode) {
  const createMode = mode === "create";
  createAccountTab.classList.toggle("active", createMode);
  createAccountTab.setAttribute("aria-selected", String(createMode));
  signInTab.classList.toggle("active", !createMode);
  signInTab.setAttribute("aria-selected", String(!createMode));
  createAccountForm.classList.toggle("hidden", !createMode);
  signInForm.classList.toggle("hidden", createMode);
  setAuthMessage("");
}

function applySession(user) {
  currentUser = user;
  accountNameDisplay.textContent = user?.name || "Guest";
...
function showAuthGate() {
  ...
  accountNameDisplay.textContent = "Guest";
}
...
function restoreSession() {
  ...
  if (sessionEmail && accounts[sessionEmail]) {
    applySession(accounts[sessionEmail]);
    return true;
  }
...
async function checkServer() {
  try {
    ...
    turnstileConfigured = Boolean(payload.turnstileConfigured);
    turnstileSiteKey = payload.turnstileSiteKey || "";

    if (turnstileConfigured && turnstileSiteKey) {
      renderTurnstile();
    } else {
      setTurnstileStatus("Bot protection is not configured yet. Live AI will stay off until Turnstile keys are added.", false);
    }

    if (payload.liveAiReady) {
      serverReady = Boolean(payload.turnstileConfigured);
      setStatus(
        serverReady
          ? `Live AI mode is on with Cloudflare Workers AI using ${payload.model || "the configured beta model"}.`
          : "Cloudflare Workers AI is connected, but bot protection is not configured yet. Live AI is paused until Turnstile is added.",
        serverReady
      );
      return;
    }
...
function resetTurnstile() {
  ...
  if (turnstileConfigured) {
    setTurnstileStatus("Complete the quick security check to unlock live AI for this message.", false);
  }
}
...
function renderTurnstile() {
  ...
  turnstileWidgetId = window.turnstile.render(turnstileWidget, {
    ...
    callback(token) {
      ...
      setTurnstileStatus("Security check complete. You can send one live AI message.", true);
    },
    "expired-callback"() {
      turnstileToken = "";
      setTurnstileStatus("Security check expired. Please complete it again.", false);
    },
    "error-callback"() {
      turnstileToken = "";
      setTurnstileStatus("Bot protection hit an error. Refresh the page and try again.", false);
    }
  });
...
async function handlePrompt(prompt) {
  ...
  try {
    ...
    if (serverReady) {
      if (!turnstileToken) {
        reply = "Complete the quick security check under the chat box before using live AI.";
      } else {
        reply = await getLiveReply(prompt);
      }
    } else {
...
function createAccount(event) {
  ...
  if (!name || !email || !password) {
    setAuthMessage("Fill out every field to create your beta account.", true);
    return;
  }
  ...
  if (accounts[email]) {
    setAuthMessage("That email already has a DollarBot beta account. Try signing in instead.", true);
    return;
  }
...
function signIn(event) {
  ...
  if (!user || user.password !== password) {
    setAuthMessage("That email and password do not match this browser's beta account.", true);
    return;
  }
